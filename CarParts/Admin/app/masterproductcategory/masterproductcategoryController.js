...
CarPartsApp.controller('masterproductcategoryController', function ($scope, $http, masterproductcategoryServices, mastermaincategoryServices, mastersubcategoryServices, $rootScope, appServices, $cookies, blockUI, $window, $q, toastr, $compile, $timeout, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, $state) {
    //====================================================================Declaration=================================================================================    
    $rootScope.categoryOpen = true;
    $scope.MasterProductCategory = {};
    ...
    //====================================================================Modal Operation=============================================================================
    $scope.openCreateMasterProductCategoryModal = function () {
        ...
        .then(function () {
            $('#MasterProductCategoryModal').modal('show');
        })        
    }

    $scope.cancelCreateMasterProductCategoryModal = function () {
        $('#MasterProductCategoryModal').modal('hide');
        $timeout(function () {
            $scope.MasterProductCategory = {};
            $scope.MasterProductCategoryForm.$setPristine();
            $scope.MasterProductCategoryForm.$setUntouched();
        }, 200)

    }

    $scope.openCategoryLogoModal = function (id) {
        masterproductcategoryServices.GetMasterProductCategoryDetails(id).then(function (response) {
            $scope.MasterProductCategory = response.data;
        })
        .then(function () {
    ...
    //====================================================================DB Operation================================================================================    
    function LoadMasterProductCategoryList() {
        masterproductcategoryServices.GetMasterProductCategoryList().then(function (response) {
            $scope.MasterProductCategoryList = response.data;
            for (let i = 0; i < $scope.MasterProductCategoryList.length; i++) {
                $scope.MasterProductCategoryList[i].AddedOn = moment.utc($scope.MasterProductCategoryList[i].AddedOn).local().format();
            }
        })
    }
    LoadMasterProductCategoryList();


    //====================================================================Event Binding===============================================================================
    $scope.UpdateMasterProductCategory = function (id, data) {
        masterproductcategoryServices.UpdateMasterProductCategory(id, data).then(function (response) {
    ...
    $scope.CreateMasterProductCategory = function () {
        $scope.MasterProductCategory.AdminId = $rootScope.AdminId;
        masterproductcategoryServices.CreateMasterProductCategory($scope.MasterProductCategory).then(function (response) {
            if (response.data.IsReport == "Success") {
                toastr.success(response.data.Message, "Success!");
            }
            else if (response.data.IsReport == "Warning") {
        ...
        .then(function () {
            ...
            .then(function () {
                $scope.cancelCreateMasterProductCategoryModal();
            })
        })
    }
    ...
    $scope.UploadMasterProductCategoryLogo = function () {
        $http({
            method: 'POST',
            url: "/Api/MasterProductCategory/UploadMasterProductCategoryLogo/" + $scope.MasterProductCategory.Id,
            headers: {
        ...
        .then(function () {
            ...
            .then(function () {
                masterproductcategoryServices.GetMasterProductCategoryDetails($scope.MasterProductCategory.Id).then(function (response) {
                    $scope.MasterProductCategory = response.data;
                })
            })
        })
